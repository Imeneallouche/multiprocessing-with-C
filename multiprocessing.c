//Made with <3 by Imène ALLOUCHE - Hadjer BOUZARA - Groupe 09 
...
{
    // The first argument is obviously the name of the executable
    // The second argument should be the name of the source file that the user should specify
    if (argc != 2) {
    ...
    snprintf(executable_file, sizeof(executable_file), "%s", base_name);
    ...
    if (pid_P1 < 0) {
        ...
        // we are inside the P1 child process
    ...
    if (WIFEXITED(status_P1) && WEXITSTATUS(status_P1) == 0) {
        ...
        if (pid_P2 < 0) {
            ...
            // we are inside the P2 child process
        ...
        if (WIFEXITED(status_P2) && WEXITSTATUS(status_P2) == 0) {
            ...
            if (WIFEXITED(status_P3)) {
                printf("P3 exited with status %d\n", WEXITSTATUS(status_P3));
